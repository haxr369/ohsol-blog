...
WHERE keyword_id IN (13) AND a.type=3
GROUP BY a.accommodation_id
HAVING COUNT(DISTINCT keyword_id) >= 1 AND
a.price BETWEEN 10000 AND 100000 AND
...
쿼리가 복잡하고 필요한 변수가 많기 때문에 Object 리스트에 쿼리에 들어갈 변수를 **순서대로** 작성했다.
